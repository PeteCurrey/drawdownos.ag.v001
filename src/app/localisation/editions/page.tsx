...
import {
  ...
  CheckCircle2, Clock, Cpu, BarChart3, Eye, ArrowRight, Database
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-[#0A0B0D] text-[#F5F6F7] p-6">
      <div className="max-w-[1600px] mx-auto space-y-6">
        ...
        <div className="grid grid-cols-4 gap-3 font-data text-xs">
          {[
            { label: 'TOTAL EDITIONS', value: '--', color: '#D6A84B' },
            { label: 'LIVE EDITIONS', value: '--', color: '#22C55E' },
            { label: 'IN REVIEW', value: '--', color: '#F97316' },
            { label: 'BLOCKING ISSUES', value: '--', color: '#EF4444' },
          ].map(s => (
        ...
        <div className="bg-[#0E1014] border border-white/8 rounded-xl overflow-hidden font-data text-xs">
          ...
          <div className="p-12 text-center space-y-3 industrial-panel">
            <Database className="w-6 h-6 text-[#626770] mx-auto" />
            <div className="font-display text-sm text-[#F5F6F7]">TMS DISCONNECTED</div>
            <div className="text-xs text-[#626770] max-w-md mx-auto">
              Please connect your Translation Management System (e.g., Lokalise, Phrase, or custom internal TMS) to fetch and display the local editions queue.
            </div>
          </div>
        </div>
